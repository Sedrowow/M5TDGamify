...
uint16_t TaskManager::addTask(const char* name, const char* details, uint8_t difficulty,
    ...
    new_task.linked_skill_category_id = linked_skill_category_id;
    new_task.linked_skill_id = linked_skill_id;
...
void TaskManager::printTask(uint16_t task_id) const {
    ...
    Serial.printf("  Repetitions: %d, Duration: %d min\n", task->repetition, task->duration_minutes);
    Serial.printf("  Reward: $%d, XP: %lu\n", task->reward_money, task->calculateXP());
    if (task->linked_skill_id > 0) {
        Serial.printf("  Linked Skill: Category #%d, Skill #%d\n",
                      task->linked_skill_category_id, task->linked_skill_id);
    }
